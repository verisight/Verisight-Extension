...
export default function NotArticleError({ url }: { url: string }) {
  ...
  return (
    <div className="w-full h-full flex flex-col items-center justify-center space-y-6 p-4 m-auto">
      ...
      <p className="leading-7 [&:not(:first-child)]:mt-6 text-center">Article doesn't seem to be a news website</p>
      <p className="leading-4 [&:not(:first-child)]:mt-6 text-xs text-center ">
        If you think this is a mistake
        press the button below to send us an email
      </p>
      <Button onClick={() => {
        window.open("mailto:support@verisightlabs.com?subject=Article%20Misclassification&body=" + mailContent);
      }}>Send mail</Button>
    </div>
  )
